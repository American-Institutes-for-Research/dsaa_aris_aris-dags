...
dag = DAG(dag_id='aris_ipeds_finance_etl',
...
def sas_finance():
    ...
    try:
        ...
        command = 'cd ' +  SERVICE_GIT_DIR + '\\SAS' + '\\d21'+ '\\Finance Survey SAS code'+' && FOR %I in (*.sas) DO sas %I'
...
def mrt_finance():
    ...
    try:
        ssh_client = ssh.get_conn()
        ssh_client.load_system_host_keys()
        command = 'cd ' +  SERVICE_GIT_DIR + '\\DB-Generation' + ' && python write_mrt_finance.py' 
        stdin, stdout, stderr = ssh_client.exec_command(command)
        out = stdout.read().decode().strip()
        error = stderr.read().decode().strip()
        print(out)
...
gen_completion_mrt = PythonOperator(
    task_id = 'load_mrt_finance',
    python_callable=mrt_finance,
    dag = dag
)

# DAG Dependancy
gen_finance >> gen_completion_mrt