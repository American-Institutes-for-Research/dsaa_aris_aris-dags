...
dag = DAG(dag_id='aris_ipeds_gr_etl',
...
def sas_gr():
    ...
    try:
        ...
        command = 'cd ' +  SERVICE_GIT_DIR + '\\SAS' + '\\d21'+ '\\GR Survey SAS code'+' && FOR %I in (*.sas) DO sas %I'
...
def mrt_GR():
    ...
    try:
        ssh_client = ssh.get_conn()
        ssh_client.load_system_host_keys()
        command = 'cd ' +  SERVICE_GIT_DIR + '\\DB-Generation' + ' && python write_mrt_gr.py' 
        stdin, stdout, stderr = ssh_client.exec_command(command)
        out = stdout.read().decode().strip()
        error = stderr.read().decode().strip()
        print(out)
...
gen_gr = PythonOperator(
    task_id='gen_sfa',
    python_callable=sas_gr,
    dag=dag
)
gen_gr_mrt = PythonOperator(
    task_id = 'load_mrt_sfa',
    python_callable=mrt_GR,
    dag= dag
)
