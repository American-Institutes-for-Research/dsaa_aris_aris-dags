...
def qc_sas_logs(qc_run):
    ...
    if(qc_run == "False"):
        ...
        command = 'cd ' +  SERVICE_GIT_DIR + '\\DB-Generation' + ' && python sas_parser.py ccd_nonfiscal_school.log' 
...
download_edge_data >> Label("Running Sas Script") >> gen_nonfiscal >>  Label("QC Checks:Sas Output")  >> qc_sas_logs >>  gen_nonfiscal_wide 
gen_nonfiscal_wide  >>  Label("QC Checks:Sas Output")  >> qc_sas_logs >> gen_nonfiscal_school 
gen_nonfiscal_school >>  Label("QC Checks:Sas Output")  >> qc_sas_logs >> qc_sas_output
