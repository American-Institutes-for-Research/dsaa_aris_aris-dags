...
def sas_enrollment():
    ...
    try:
        ...
        command = 'cd ' +  SERVICE_GIT_DIR + '\\SAS' + '\\d21'+ '\\Enrollment Survey SAS code'+' && FOR %I in (*.sas) DO sas %I'
...
def mrt_enrollment():
    ...
    try:
        ssh_client = ssh.get_conn()
        ssh_client.load_system_host_keys()
        command = 'cd ' +  SERVICE_GIT_DIR + '\\DB-Generation' + ' && python write_mrt_enrollment.py' 
        stdin, stdout, stderr = ssh_client.exec_command(command)
        out = stdout.read().decode().strip()
        error = stderr.read().decode().strip()
        print(out)
...
gen_enrollment_mrt = PythonOperator(
    task_id='load_mrt_enrollment',
    python_callable=mrt_enrollment,
    dag=dag
)

# DAG Dependancy
gen_enrollment  >> gen_enrollment_mrt